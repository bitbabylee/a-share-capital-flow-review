...
if (D.flow && D.flow.l1_in && D.flow.l1_in.length) (function(){
  ...
  const block = (title, arr, isIn)=>{
    if (!arr || !arr.length) {
      const src = D.flow_src === 'eastmoney' ? '东方财富公开接口' : '当前数据源';
      return `<div class="card"><h3>${title}</h3><div class="empty-state">${src}未返回该榜单<br>已保留空值，未使用推算数据</div></div>`;
    }
...
function lhbTable(id, arr, isBuy){
  const rows = arr.map(x=>`<tr><td>${x.name}${x.code?`<span style="color:var(--sub);font-size:11.5px"> ${x.code}</span>`:`<span style="color:var(--sub);font-size:11.5px"> 代码待核验</span>`}</td>
